// ObliviousGmn // Dokuu // October 2020 // Keymaps

#include QMK_KEYBOARD_H

#include "layers.h"
#include "logo.h"
#include "oleds.h" 
#include "brain.c"

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {

  [_QWERTY] = LAYOUT( \
    // |----------+----------+----------+----------+----------+----------\                       /----------+----------+----------+----------+----------+----------|
        KC_ESC,    KC_1,      KC_2,      KC_3,      KC_4,      KC_5,                              KC_6,      KC_7,      KC_8,      KC_9,      KC_0,      KC_GRV, \
    // |----------+----------+----------+----------+----------+----------\                       /----------+----------+----------+----------+----------+----------|
        KC_TAB,    KC_Q,      KC_W,      KC_E,      KC_R,      KC_T,                              KC_Y,      KC_U,      KC_I,      KC_O,      KC_P,      KC_BSLS, \
    // |----------+----------+----------+----------+----------+----------\                       /----------+----------+----------+----------+----------+----------|
        KC_LCTL,   KC_A,      KC_S,      KC_D,      KC_F,      KC_G,                              KC_H,      KC_J,      KC_K,      KC_L,      KC_SCLN,   KC_QUOT, \
    // |----------+----------+----------+----------+----------+----------+----------\ /----------+----------+----------+----------+----------+----------+----------|
        KC_LSFT,   KC_Z,      KC_X,      KC_C,      KC_V,      KC_B,      KC_LBRC,      KC_RBRC,  KC_N,      KC_M,      KC_COMM,   KC_DOT,    KC_SLSH,   KC_ENT, \
    // |----------+----------+----------+----------+----------+----------+----------\ /----------+----------+----------+----------+----------+----------+----------|
                                         KC_LALT,   KC_LGUI,  LT(2,KC_SPC), KC_SPC,  KC_ENT, LT(2,KC_SPC), LT(1,KC_BSPC), KC_APP
    //                                  \----------+----------+----------+----------\ /----------+----------+----------+----------/
  ),

  [_LOWER] = LAYOUT( \
    // |----------+----------+----------+----------+----------+----------\                       /----------+----------+----------+----------+----------+----------|
        _______,   _______,   _______,   _______,   _______,   _______,                           _______,   _______,   _______,   KC_MINS,   KC_EQL,    KC_PSCR, \
    // |----------+----------+----------+----------+----------+----------\                       /----------+----------+----------+----------+----------+----------|
        KC_F1,     KC_F2,     KC_F3,     KC_F4,     KC_F5,     KC_F6,                             KC_F7,     KC_F8,     KC_F9,     KC_F10,    KC_F11,    KC_F12, \
    // |----------+----------+----------+----------+----------+----------\                       /----------+----------+----------+----------+----------+----------|
        _______,   _______,   _______,   _______,   _______,   _______,                           KC_INS,    KC_HOME,   KC_UP,     KC_END,    KC_PGUP,   _______, \
    // |----------+----------+----------+----------+----------+----------+----------\ /----------+----------+----------+----------+----------+----------+----------|
        _______,   _______,   _______,   _______,   _______,   _______,   _______,     _______,   KC_DEL,    KC_LEFT,   KC_DOWN,   KC_RGHT,   KC_PGDN,   _______, \
    // |----------+----------+----------+----------+----------+----------+----------\ /----------+----------+----------+----------+----------+----------+----------|
                                         _______,   _______,   _______,   _______,     _______,   _______,   _______,   _______
    //                                  \----------+----------+----------+----------\ /----------+----------+----------+----------/
  ),

  [_RAISE] = LAYOUT( \
    // |----------+----------+----------+----------+----------+----------\                       /----------+----------+----------+----------+----------+----------|
        _______,   _______,   _______,   _______,   _______,   _______,                           KC_VOLD,   KC_VOLU,   KC_NLCK,   KC_PSLS,   KC_PAST,   KC_PMNS, \
    // |----------+----------+----------+----------+----------+----------\                       /----------+----------+----------+----------+----------+----------|
        _______,   _______,   _______,   _______,   _______,   _______,                           KC_MUTE,   KC_MPLY,   KC_P7,     KC_P8,     KC_P9,     KC_PPLS, \
    // |----------+----------+----------+----------+----------+----------\                       /----------+----------+----------+----------+----------+----------|
        _______,   _______,   _______,   _______,   _______,   _______,                           KC_MPRV,   KC_MNXT,   KC_P4,     KC_P5,     KC_P6,     KC_PDOT, \
    // |----------+----------+----------+----------+----------+----------+----------\ /----------+----------+----------+----------+----------+----------+----------|
        _______,   _______,   _______,   _______,   _______,   _______,   _______,     _______,   _______,   _______,   KC_P1,     KC_P2,     KC_P3,     KC_PENT, \
    // |----------+----------+----------+----------+----------+----------+----------\ /----------+----------+----------+----------+----------+----------+----------|
                                         _______,   _______,   _______,   _______,      _______,  _______,   _______,   KC_P0
    //                                  \----------+----------+----------+----------\ /----------+----------+----------+----------/
  ),

};
